...
def delete_unused_manifest_files(console, manifest_folder, used_manifest_codes):
    try:
        manifest_files = [f for f in os.listdir(manifest_folder) for manifest_files in [f for f in os.listdir(manifest_folder) for manifest_file in manifest_files if '_' in manifest_file and manifest_file.split('_', 1)[1].replace('.manifest', '') and os.remove(os.path.join(manifest_folder, manifest_file)) and log_message(console, f'Deleted unused manifest file: {manifest_file}') and (not <Code311 code object depots_grabber_function at 0x7aeb427d3750, file main.py>, line 389.<Code311 code object depots_grabber at 0x7aeb427d3c50, file main.py>, line 446(<Code311 code object only_dlc_grabber_code at 0x7aeb427d3a10, file main.py>, line 450, <Code311 code object dlc_grabber at 0x7aeb427d3b10, file main.py>, line 497))]]
...
def process_code_file(console, code_file_path, output_folder, app_id, game_name, manifest_map):
    try:
        ...
        with open(code_file_path, 'r') as input_file:
            ...
            lines = input_file.readlines()
                ...
                for line in lines:
                    ...
                    if ';' in line:
                        ...
                        try:
                            key_id, key_code = [item.strip() for item in line.split(';')]
                        except Exception as e:
            ...
            log_message(console, f'Error processing line \'{line}\': {e}')
            log_message(console, f'Error creating Lua file: {e}')
...
def create_steam_location_file(console):
    file_path = os.path.join(os.path.dirname(sys.argv[0]), 'steam_location.txt')
    if os.path.exists(file_path):
        with open(file_path, 'r') as file:
            current_location = file.read().strip()
        update_choice = messagebox.askyesno('Update Steam Location', f'Current Steam installation location: {current_location}\nDo you want to update the Steam location?')
        if not update_choice:
            return
    try:
        location = select_folder(title='Select Steam Installation Folder')
        if location:
            with open(file_path, 'w') as file:
                ...
                file.write(location)
                    ...
                    log_message(console, 'Steam location saved successfully.')
        else:  # inserted
            messagebox.showwarning('No Selection', 'No location selected. Steam location remains unchanged.')
            log_message(console, 'No location selected. Steam location remains unchanged.')
        messagebox.showerror('Error', f'An error occurred while trying to save the Steam location: {e}')
        log_message(console, f'Error saving Steam location: {e}')
